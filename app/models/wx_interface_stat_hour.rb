class WxInterfaceStatHour < ActiveRecord::Base
  def self.wx_data_pull(wx_official_account, url)
    ...
    if result.is_ok?
      result.result[:list].each do |item|
        wx_interface_stat_hour = WxInterfaceStatHour.new(wx_official_account_id: wx_official_account.id,
                                                         date: item[:ref_date], hour: item[:ref_hour],
                                                         callback_count: item[:callback_count],
                                                         fail_count: item[:fail_count],
                                                         total_time_cost: item[:total_time_cost],
                                                         max_time_cost: item[:max_time_cost])
        wx_interface_stat_hour.save
      end
    end
  end
end
